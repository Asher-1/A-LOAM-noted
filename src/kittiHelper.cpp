...
void read_lidar_data(const std::string lidar_data_path,
...
int main(int argc, char** argv) {
  ...
  while (std::getline(timestamp_file, line) && ros::ok()) {
    ...
    read_lidar_data(lidar_data_path.str(), lidar_data);
    ...
    for (std::size_t i = 0; i < lidar_data.size(); i += 4) {
      ...
      // lidar_data[i+2]); lidar_intensities.push_back(lidar_data[i+3]);
      // 构建pcl的点云格式
      pcl::PointXYZI point;
